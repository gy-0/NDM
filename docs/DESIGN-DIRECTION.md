...
第二节测出「四处取样完全相同」之后，做的不是把 Mole 的暖色搬过来（那条被否了），
而是给窗口建立**图底关系**——这是那组数据本身指出的问题，与色相无关。

浅色主题原本三个面都是 `#FAFBFD`，注释写着"one seamless paper canvas"。
而深色主题一直有层次（`#0A0C11` 导轨 / `#12161E` 内容）——**是浅色把它丢了**。

现在浅色对齐深色的结构：

| 面 | 改前 | 改后 | L |
|---|---|---|---|
| 侧边栏 / 检查器（导轨） | #FAFBFD | **#F2F4F8** | 244 |
| 工具条 | #F9FAFD | #F7F9FC | 249 |
| 列表（主体） | #FAFBFD | **#FFFFFF** | 255 |

检查器也归到导轨一侧——它是**关于**内容的框，不是内容本身。窗口因此读作
"一张明亮的纸被托在凹陷的框里"，而不是一整片没有主次的平面。
