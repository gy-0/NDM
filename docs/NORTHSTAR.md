...
| C2-4 | 接入：入库 + 删除清理 | 🟡 2026-07-24 入库与清理已接完（10 个测试）。**启动按需重建未做**，见 C2-7 |
| C2-7 | **从磁盘重建索引** | ⬜ 需要一个 SRT 解析器：`.txt` 文稿**没有时间轴**，只从它重建会静默丢掉"跳到那一秒"这个头号能力——那是降级不是恢复。所以重建要读 `.srt`，得先写解析器（纯函数、易测） |
