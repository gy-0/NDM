...
| A7 | **`remove` 失败时状态不一致** —— `DownloadManager.remove` 的 `defer` 在抛异常时照样执行，会清掉引擎注册表并删除任务的 support 目录，而 DB 行还留着 | ⬜ App 总会注入 recycler 所以线上难触发，但"行还在、segments 已删"的任务重启后看着可续传实际不能。发现于 A3 压测 |
