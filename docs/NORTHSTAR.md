...
| C7 | **直播录制** | 🟡 2026-07-24 引擎层已完成（识别 live、轮询、去重、追加、可停止、时长/体积上限）。**预约定时未做**；界面未做 |
...
| C3-2 | 模型摘要与章节标题 | ✅ 2026-07-24 `TranscriptNarrator`（`@Generable` 引导输出、超时、永不抛出），9 个测试 |
| C3-3 | CLI 呈现 | ✅ 2026-07-24 `ndmcli chapters <file>`（Markdown 大纲 + 摘要 + `--json`），13 个测试 |
...
| 2026-07-24 | C7 直播录制（引擎层） | 用户指出直播捕获可能在 `reverse/` 里有旧实现。查证结论相反：二进制里**没有 `#EXT-X-ENDLIST` 这个字符串**，`hlsSegmentsCount` 是固定存储属性，扩展的 `HlsParser` 只解析一次——原版和它的扩展都**没有**直播捕获循环。所以这是全新能力，不是找回旧功能（对差异化反而是好消息：连 2021 那版都没有）。**顺带修掉一个现存的静默失败**：`endList` 一直被解析但从没被使用，把直播流丢给 NDM 会得到窗口里那几十秒的残片并报告成功。核心难点是**分片身份**：滚动窗口里数组下标每次刷新都指向不同分片，按下标去重必然重复下载或漏片。做成纯逻辑 `LiveSegmentTracker`（17 测试）按 media sequence 识别，并**验证了承重假设**——用真实解析器确认 `Segment.id` 就是 media sequence（`#EXT-X-MEDIA-SEQUENCE:8471` → id 8471/8472/8473），若它其实是下标则整套逻辑失效。三个刻意的设计：下载失败**不推进游标**（分片还在窗口里，下次刷新要能重试）；窗口滑过未取的分片**计数为缺口**并说出来（有洞的录像不能看着像完整的）；播放列表刷新失败**不中止**（直播会打嗝，为一次坏响应丢掉整场录制不值）。录制无界所以强制上限（默认 4 小时 / 8 GB）——没有上限的话一次忘关就填满磁盘。7 个集成测试用一个真会滑动的测试 origin，含一条断言 VOD 路径行为不变。727 passed |
| 2026-07-24 | C3-2/C3-3 摘要与 CLI 出口 | 用 `@Generable` 引导输出而不是解析自由文本（中文没有空格可锚定、标点也不保证）。实测 1.64s、摘要 21 字、标题 8/6/6 字、数量与章节一致。`TranscriptNarrator` **永不抛出**——摘要失败就退化，为了一句摘要丢掉整份文稿是荒谬的交换；带超时（模型调用无上界）；**标题数量不匹配时整批丢弃而不是对齐**，否则每个章节都会挂上别人的标题。CLI 加 `ndmcli chapters`，实测真实中文语音→转写→带 AI 标题的大纲+摘要+JSON 全通。**门禁抓到一个可复现的真 bug**：`testPreparingAnInstalledLanguage…` 报 `unsupportedLanguage`，而它测的是**已安装**语言。探针发现：已安装的 11 个语言里**前 5 个成功、之后全抛错码 11**——是 `maximumReservedLocales: 5` 的预留额度耗尽，而我把**任何**抛错都映射成"不支持这个语言"。错误的诊断比没有诊断更糟：它把用户指向一个根本不存在的问题。两处都修：`prepare` 先查 `readiness`、已就绪直接返回（既让两处判断一致，也让常见路径根本不花预留额度）；错误映射只在错误码 4（真的不支持）时才说不支持，其余报 `.temporarilyUnavailable`。加了跨越额度上限的回归测试。门禁连跑两次 703 passed。**8h 长跑 t+4202s**：13172 轮、RSS 从 184.6 **降到** 180.9 MB（有回收，不是泄漏）、fd 14、rows 0 |
